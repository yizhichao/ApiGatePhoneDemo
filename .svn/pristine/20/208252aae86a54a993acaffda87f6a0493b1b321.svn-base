...
public class AudioVideoActivity extends AppCompatActivity implements PlayerManager.PlayerStateListener,
    ...
    public static final int PLAYBACK_RESULT = 888;
    /**
     * 上次选择的控制命令
     */
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                case PLAYBACK_RESULT:
                    if (msg.obj != null) {
                        String playbackUrl = String.valueOf(msg.obj);
                        if (playbackUrl.contains("rtsp")) {
                            Intent intent = new Intent(AudioVideoActivity.this, AudioVideoActivity.class);
                            intent.putExtra("URL", playbackUrl);
                            intent.putExtra("showPTZ", false);
                            startActivity(intent);
                        } else {
                            UIUtil.showToast(AudioVideoActivity.this, "获取录像回放RTSP失败");
                        }
                    } else {
                        UIUtil.showToast(AudioVideoActivity.this, "获取录像回放RTSP失败");
                    }
    ...
    private void showPalyBackDialog() {
        RelativeLayout relativeLayout = (RelativeLayout) getLayoutInflater().inflate(R.layout.dialog_playback, null);
        AlertDialog.Builder builder = new AlertDialog.Builder(AudioVideoActivity.this);
        builder.setView(relativeLayout);
        etBeginTime = (EditText) relativeLayout.findViewById(R.id.et_begin_time);
        etEndTime = (EditText) relativeLayout.findViewById(R.id.et_end_time);
        etbCascade = (EditText) relativeLayout.findViewById(R.id.et_bCascade);
        ...
        builder.setPositiveButton("确定", new DialogInterface.OnClickListener() {
            ...
            public void onClick(DialogInterface dialog, int which) {
                ...
                if (TextUtils.isEmpty(beginTime) || TextUtils.isEmpty(endTime)) {
                    UIUtil.showToast(AudioVideoActivity.this, "参数不完整");
                } else {
                    mLiveControl.playBack(indexcode, beginTime, endTime, bCascade);
                }
            }
        });
